...
taxa_database_IDTAXA <- "../../../../taxanomy_databases/UNITE_v2020_February2020.rdata"
...
#taxa with RDP classifer
taxa_rdp <- assignTaxonomy(seqtab.nochim, taxa_database, multithread=TRUE, tryRC = TRUE, minBoot = 30) #try in both directions

#taxa with IDTAXA classifier
dna <- DNAStringSet(getSequences(seqtab.nochim)) # Create a DNAStringSet from the ASVs
load(taxa_database_IDTAXA) # CHANGE TO THE PATH OF YOUR TRAINING SET
ids <- IdTaxa(dna, trainingSet, strand="top", processors=NULL, verbose=FALSE, threshold = 40) # use all processors
ranks <- c("kingdom", "phylum", "class", "order", "family", "genus", "species") # ranks of interest
# Convert the output object of class "Taxa" to a matrix analogous to the output from assignTaxonomy
taxa_id <- t(sapply(ids, function(x) {
...
rownames(taxa_id) <- getSequences(seqtab.nochim)
colnames(taxa_id) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")

#overwrite unidentified taxa and taxa whre only species is assigned with NA
unidentified <- apply(taxa_id, 2, function(x) grepl("unidentified",x))
taxa_id[unidentified] <- NA
only_sp <- is.na(taxa_id$Genus) & !(is.na(taxa_id$Species))
taxa_id$Species[only_sp] <- NA
...
#replace the sequence headers by the correct ASV number in taxa_rdp
for (i in 1:nrow(taxa_rdp)){
  for (j in 1:nrow(ASV_seq)){
    if(rownames(taxa_rdp)[i] == ASV_seq[j,2]){
      rownames(taxa_rdp)[i] <- ASV_seq[j,1]
    }
  }
}
rownames(taxa_rdp) <- gsub(">", "", rownames(taxa_rdp))

#replace the sequence headers by the correct ASV number in taxa_id
for (i in 1:nrow(taxa_id)){
  for (j in 1:nrow(ASV_seq)){
    if(rownames(taxa_id)[i] == ASV_seq[j,2]){
      rownames(taxa_id)[i] <- ASV_seq[j,1]
    }
  }
}
rownames(taxa_id) <- gsub(">", "", rownames(taxa_id))


#"UTAX db" has different FASTA headers than "general release db"
#taxomony df needs to be done manually in taxa_rdp

#empty df
temp <- data.frame(matrix(NA, nrow(taxa_rdp), 7))
rownames(temp) <- rownames(taxa_rdp)
colnames(temp) <- c("Kingdom","Phylum","Class","Order","Family","Genus","Species")

#extract taxa_rdp
for(tax in 1:nrow(taxa_rdp)){
  tax_string <- paste0(taxa_rdp[tax,2],",") #extract string and add "," at the end
  tax_vector <- qdapRegex::ex_between(tax_string, ":", ",")[[1]] #use regex to extract taxa_rdp
  temp[tax, 1:length(tax_vector)] <- tax_vector #add taxa_rdp
}
#remove db-number
temp[,7] <- gsub("_SH[0-9]*.08FU", "", temp[,7])
#overwrite taxa_rdp df
taxa_rdp <- temp; rm(temp)

#combine both classifiers
taxa <- taxa_rdp; taxa[,] <- NA #empty df
taxa$classifier <- NA

for(i in 1:nrow(taxa)){
  n_RDP <- sum(!(is.na(taxa_rdp[i,])))
  n_IDTAXA <- sum(!(is.na(taxa_id[i,])))
  if(n_RDP >= n_IDTAXA){tax <- taxa_rdp[i,]; classifier <- "RDP"} #more assigned taxa-levels in RDP
  else{tax <- taxa_id[i,]; classifier <- "IDTAXA"} #same or more assigned taxa-levels in IDTAXA
  taxa[i,1:7] <- tax
  taxa$classifier[i] <- classifier
}

#save taxonomy assignments
saveRDS(taxa_rdp, paste(path.rds, "taxa_RDP.RDS", sep=""))
saveRDS(taxa_id, paste(path.rds, "taxa_IDTAXA.RDS", sep=""))
...
for (n in c("97", "98", "99")) {
  ...
  #OTU taxa
  OTU_taxa <- data.frame(matrix(nrow = nrow(OTU_ASV_seq), ncol = 8))
